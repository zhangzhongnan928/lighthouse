...
fuzz_target!(|data: &[u8]| {
    ...
    if attestation.is_err() {
        ...
        let (block, mut state) = build_beacon_state_and_block(&spec);
...
fn build_beacon_state_and_block(spec: &ChainSpec) -> (BeaconBlock, BeaconState<MinimalEthSpec>) {
    ...
    // Test Attestation SSZ Round trip
    let attestation = block_builder.block.body.attestations[0].clone();
    let bytes = attestation.as_ssz_bytes();
    println!("Attestation SSZ hex: {}", hex::encode(&bytes));
    let attestation_round_trip = Attestation::from_ssz_bytes(&bytes).unwrap();
